...
- [x] T002: 多言語ルーティング基盤の実装
- [x] T003: 共通レイアウト（Header/Footer）の実装

### Phase 2: ページ・コンポーネント実装

- [x] T004: アニメーション基盤コンポーネントの実装
- [x] T005: トップページの実装
- [x] T006: 会社概要ページの実装
- [x] T007: お問い合わせページ・フォームの実装
- [x] T008: 多言語辞書ファイル（ja.json / zh.json）の作成
...
- 完了条件:
  - [x] `/ja/` と `/zh/` でそれぞれ対応言語のページが表示される
  - [x] `/` アクセスで `/ja/` にリダイレクトされる
  - [x] `generateStaticParams` で `ja`, `zh` の静的パスが生成される
  - [x] 辞書の読み込み関数が動作する
...
- 完了条件:
  - [x] ヘッダーにロゴ、ナビゲーション、言語切替ボタンが表示される
  - [x] フッターに会社情報、コピーライト、ページリンクが表示される
  - [x] 言語切替ボタンで `/ja/` ⇄ `/zh/` を遷移できる
  - [x] モバイルでハンバーガーメニューに切り替わる
...
- 完了条件:
  - [x] `FadeInSection` でスクロール時のフェードインが動作する
  - [x] `ParallaxHero` でパララックス効果が動作する
  - [x] `PageTransition` でページ遷移アニメーションが動作する
  - [x] `prefers-reduced-motion` 設定時にアニメーションが無効化される
...
- 完了条件:
  - [x] ファーストビューにアニメーション付きヒーローセクションが表示される
  - [x] サービス概要（投資コンサル/仲介/内見サポート）が3カラムで表示される
  - [x] CTAボタンからお問い合わせページに遷移できる
  - [x] 日本語・中国語で正しいテキストが表示される
...
- 完了条件:
  - [x] 会社基本情報（日中不動産パートナーズ株式会社、代表: 太田翔子、所在地等）が表示される
  - [x] 企業理念・ミッションセクションが表示される
  - [x] スクロールアニメーションが適用されている
  - [x] 日本語・中国語で正しいテキストが表示される
- 並列実行: T005 と同時実行可能
...
- 完了条件:
  - [x] フォームに氏名、メール、電話番号（任意）、種別、内容の入力欄がある
  - [x] `data-netlify="true"` と `netlify-honeypot` が設定されている
  - [x] 必須項目チェック、メール形式バリデーションが動作する
  - [x] 送信完了メッセージが表示される
  - [x] 日本語・中国語でフォームラベルが正しく表示される
...
- 対象ファイル:
  ...
  - `src/i18n/zh.json`
- 完了条件:
  - [x] 全ページのテキストが辞書ファイルに定義されている
  - [x] 日本語と中国語の辞書構造が一致している
  - [x] ハードコードされたテキストがない
